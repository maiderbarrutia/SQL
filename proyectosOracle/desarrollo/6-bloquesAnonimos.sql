...
BEGIN
    FOR uf_rec IN c_uf
    LOOP
        notaMedia := uf_rec.NOTA_MEDIA_UF;

        UPDATE UFS
        SET
            NOTA_FINAL_UF = notaMedia * 0.4 + 0.6 * 7, -- Reemplazar 7 con el valor deseado para NOTA_EXAMEN
            STAT_UF = CASE
                          WHEN notaMedia >= 5 THEN 'APROBADA'
                          ELSE 'SUSPENDIDA'
                      END
...
BEGIN
    ...
    LOOP
        ...
        UPDATE UFS
        SET
            NOTA_FINAL_UF = notaMedia * 0.4 + 0.6 * uf_rec.NOTA_EXAM,
            STAT_UF = CASE
                          WHEN notaMedia >= 5 THEN 'APROBADA'
                          ELSE 'SUSPENDIDA'
                      END
